...
function sostavChisla(massivChisel, chislo) {
  //я бы переименовал massivChisel на numbersList/Array или candidates
  //а chislo на target
  const result = [];
  ...
  function calculate(arr = [], sum = 0, idx = 0) {
    arr = [...new Set(arr)];

    if (sum > chislo) return;
    if (sum === chislo && sumArray(arr) === chislo) result.push(arr.sort());
    for (let i = idx; i < massivChisel.length; i++) {
      calculate([...arr, massivChisel[i]], sum + massivChisel[i], i);
    }
  }
  calculate();
  return result;
